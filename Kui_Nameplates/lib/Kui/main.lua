...
kui.GetUnitColour = function(unit, str)
    ...
    if (UnitIsTapped(unit) and not UnitIsTappedByPlayer(unit)) or UnitIsDeadOrGhost(unit) or not UnitIsConnected(unit) then
        ret = {r = .5, g = .5, b = .5}
    else
        ...
        if info.startDelay and info.startDelay > 0 then
            ...
            if info.fadeTimer < info.timeToFade then
                ...
                if info.mode == "IN" then
                    frame:SetAlpha(
                        (info.fadeTimer / info.timeToFade) * (info.endAlpha - info.startAlpha) + info.startAlpha
                    )
                elseif info.mode == "OUT" then
                    frame:SetAlpha(
                        ((info.timeToFade - info.fadeTimer) / info.timeToFade) * (info.startAlpha - info.endAlpha) +
                            info.endAlpha
                    )
                end
            else
